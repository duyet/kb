...
- [Cloudflare Workers Cache sits in front of your Worker](tech-cloudflare-workers-cache.md) — Per-worker tiered cache; `cache.enabled` in wrangler + standard `Cache-Control`/`Cache-Tag`/SWR headers; a HIT skips the Worker (no CPU billing); authed requests auto-bypass
