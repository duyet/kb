...
- [Cloudflare acquired Astro](memory/reference-cloudflare-acquires-astro.md) — Jan 2026; Astro team (Fred Schott) now at CF, ships Flue agent framework
...
- [ClickHouse multi-host setup](memory/projects/homelab/reference-clickhouse-machines.md) — three instances (k3s primary, small cloud clone, high-mem VPS); memory tuning + cross-host clone lessons
- [ClickHouse multi-host session log](memory/projects/homelab/sessions/project-clickhouse-machines-setup.md) — connectivity fixes, memory optimization, cross-host data clone
...
- [chmonitor — one codebase OSS + Cloud SaaS](memory/project-chmonitor-one-codebase-saas.md) — self-host + hosted from one codebase; single-source .env pattern keeps Wrangler/Docker/K8s in sync
